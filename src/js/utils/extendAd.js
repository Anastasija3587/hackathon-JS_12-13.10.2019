...
export const extendAdWindow = async evt => {
  ...
  const request = await axios.get(
    `https://dash-ads.goit.co.ua/api/v1/ads/${id}`
  );
  ...
  const getDataParams = extendedAd({
    id: getData._id,
    type: getData.title,
    phone: getData.phone,
    createdAt: dateformat(createdAt, "dddd, mmmm dS, yyyy, h:MM:ss TT"),
    updatedAt: dateformat(updatedAt, "dddd, mmmm dS, yyyy, h:MM:ss TT"),
    img: getData.images,
